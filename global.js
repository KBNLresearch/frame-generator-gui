$(function() {
    ...
    $("form").submit(function(e) {
        ...
        for (var i = 0; i < files.length; i++) {
          ...
          data.append('doc_files[]', file, file.name);
        }
...
function transform(data) {
    ...
    graph_data = {'nodes': [], 'links': [], 'labelAnchors': [], 'labelAnchorLinks': []}

    for (var item in json['frames']) {
        for (var keyword in json['frames'][item]['keyword']) {
            console.log(keyword)
            console.log(json['frames'][item]['keyword'][keyword])
            node = {'label': keyword, 'group': 1, 'score': json['frames'][item]['keyword'][keyword]}
    ...
    for (var item in json['frames']) {
        for (var frame in json['frames'][item]['frame']) {
            for (var frame_word in json['frames'][item]['frame'][frame]) {
                ...
                if (!id_exists) {
                    node = {'label': frame_word, 'group': 2, 'score': 0.25}
                    graph_data['nodes'].push(node)
                    graph_data['labelAnchors'].push({'node': node})
                    graph_data['labelAnchors'].push({'node': node})
                }
                for (var keyword in json['frames'][item]['keyword']) {
                    ...
                    graph_data['links'].push({'source': frame_word_node_id, 'target': keyword_node_id, 'score': json['frames'][item]['frame'][frame][frame_word]})
                }
            }
        }
    }
...
function draw(graph) {
    ...
    var color = ['#', "#a8ddb5", "#2b8cbe"]

    console.log(nodes)

    var force = d3.layout.force().size([w, h]).nodes(nodes).links(links).gravity(1).linkDistance(90).charge(-3000).linkStrength(5);

    force.start();

    var force2 = d3.layout.force().nodes(labelAnchors).links(labelAnchorLinks).gravity(0).linkDistance(0).linkStrength(5).charge(-100).size([w, h]);

    force2.start();

    var link = vis.selectAll("line.link").data(links).enter().append("svg:line").attr("class", "link").style("stroke-width", function(d) { return 15 * d.score; });

    var node = vis.selectAll("g.node").data(force.nodes()).enter().append("svg:g").attr("class", "node");
    node.append("svg:circle").attr("r", function(d) { return 25 * d.score; }).attr("fill", function(d) { return color[d.group]; });
